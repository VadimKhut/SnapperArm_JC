...
void setup(){
	...
	{
		sBase = 1500;
		doArmIK(false, g_sBase,g_sIKY,g_sIKZ,g_sIKGA);  // 1500; Y=150; Z=150; GA=0
	}
...
void loop(){
	...
	buttonState1 = digitalRead(BUTTON1);	// Capture Pushbutton
	//buttonState2 = digitalRead(BUTTON2);	// Playback Pushbutton - used in playback firmware 
...
void MenuOptions(){
	...
	Serial.println("###########################"); 
	Serial.println("Please enter 1, or press Button 'Capture'"); 
	Serial.println("Start Analog IK Control Mode");        
	Serial.println("###########################"); 
}
...
void AnalogControlLoop(){
	...
	Serial.println("Send '1' or press the 'Capture' Button to pause the joysticks and capture the current pose."); 
	...
	if(Serial.available() > 0)
		inByte = Serial.read(); // Read & discard the character that got us out of the loop.
